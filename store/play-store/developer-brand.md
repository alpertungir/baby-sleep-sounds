# Tngr Studio — Play Console marka metinleri

Play Console → **Geliştirici hesabı** / **Store presence** / **Developer page** alanlarına kopyala-yapıştır.

---

## Geliştirici adı (Developer name)

```
Tngr Studio
```

**Stüdyo ikonu:** `store/tngr-studio/icon-512.png` — sade wordmark (512×512, opak PNG — Play geliştirici profili)

**Kapak görseli:** `store/tngr-studio/developer-header-4096x2304.png` — Play geliştirici sayfası header

> Play Console → Ayarlar → Geliştirici hesabı → Geliştirici profili

---

## Geliştirici açıklaması — Türkçe

```
Tngr Studio, Alper Tıngır tarafından kurulan bağımsız bir mobil uygulama stüdyosudur.

Aileler ve ebeveynler için sade, reklamsız ve gizlilik odaklı uygulamalar geliştiriyoruz. İlk odak alanımız bebek uyku rutini: ninni, beyaz gürültü ve rahatlatıcı sesler.

Bebek Uyku Sesleri uygulamamız kişisel veri toplamaz; favoriler yalnızca cihazda saklanır.
```

---

## Developer description — English

```
Tngr Studio is an independent mobile app studio founded by Alper Tıngır.

We build simple, ad-free, privacy-focused apps for families and parents. Our first focus is bedtime routines for babies: lullabies, white noise, and calming sounds.

Baby Sleep Sounds does not collect personal data; favorites are stored on the device only.
```

---

## Web sitesi / iletişim

| Alan | Değer |
|------|--------|
| E-posta | alperttungir@gmail.com |
| Web | https://alpertungir.github.io/baby-sleep-sounds/ |
| Gizlilik politikası | https://alpertungir.github.io/baby-sleep-sounds/privacy-policy.html |

---

## Store listing — kısa alanlar (TR)

**Uygulama adı:** Bebek Uyku Sesleri  
**Kısa açıklama (80 karakter max):**
```
Bebek uyku sesi: ninni, beyaz gürültü, rahatlatıcı melodiler. Reklamsız, 50+ ses.
```
*(79 karakter — ASO anahtar kelimeleri: bebek uyku sesi, ninni, beyaz gürültü)*

---

## Store listing — short fields (EN)

**App name:** Baby Sleep Sounds  
**Short description (80 chars max):**
```
Baby sleep sounds: lullabies, white noise & calming nursery audio. 50+ sounds, ad-free.
```
*(78 characters)*

---

## Sürüm notları — 1.0.0 (TR)

```
İlk sürüm!

• 4 kategori, 50+ rahatlatıcı ses
• 8 dil desteği
• Uyku zamanlayıcısı, favoriler ve çalma listesi
• Kilit ekranı ve bildirimden müzik kontrolü
• Beyaz gürültü ve Türk ninneleri çevrimdışı
• Reklamsız, gizlilik odaklı
```

---

## Release notes — 1.0.0 (EN)

```
First release!

• 4 categories, 50+ calming sounds
• 8 languages
• Sleep timer, favorites and playlist
• Lock screen and notification controls
• White noise and Turkish lullabies offline
• Ad-free and privacy-focused
```

---

## ASO

Detaylı Play Store sıralama rehberi: [`aso-guide.md`](aso-guide.md)
